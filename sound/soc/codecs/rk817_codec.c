...
static struct rk817_reg_val_typ playback_power_up_list[] = {
	...
	{RK817_CODEC_DDAC_VOLL, 0x32},
	{RK817_CODEC_DDAC_VOLR, 0x32},
};
...
static DECLARE_TLV_DB_SCALE(rk817_spk_volume_tlv, 0, 1, 1);
...
static int rk817_spk_volume_get_tlv(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    return snd_soc_get_volsw_range(kcontrol, ucontrol);
}

static int rk817_spk_volume_set_tlv(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
    struct rk817_codec_priv *rk817 = snd_soc_component_get_drvdata(component);
    ...
    if (volume > 255) {
        DBG("%s : invalid SPK volume: %d\n", __func__, volume);
        return -EINVAL;
    }

    rk817->spk_volume = volume;
    rk817->hp_volume = volume;
	DBG("%s set spk volume %d\n", __func__, volume);
...
static struct snd_kcontrol_new rk817_snd_path_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("HPL Volume",
            RK817_CODEC_DDAC_VOLL,
            0, 255, 1,
            rk817_spk_volume_get_tlv,
            rk817_spk_volume_set_tlv,
            rk817_spk_volume_tlv),
    SOC_SINGLE_EXT_TLV("HPR Volume",
