...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	i = gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
	GTP_INFO("I2C TRANSFER GTP_REG_VERSION : %d", i);
...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	g_chipid = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	...
	ret = gt1x_i2c_read_dbl_check(GTP_REG_CONFIG_DATA, buf, 1);
	g_fwversion = buf[0];
	return 0;
}
